...
async function findByID(id) {
    const findByIDQuery = `
    SELECT * FROM "platform_families".index 
    where id = $1;
  `;
    return new Promise(async (resolve, reject) => {
      pool.query(findByIDQuery, [id]).then((findResult) => {
        ...
        reject(`Unable to find Platform Family with ID: '${id}'`)
      }).catch(error => {
        reject(error)
      });
    });
  ...
  async function create(id, name, slug) {
    const createPlatformFamilyQuery = `
      INSERT INTO "platform_families".index (id, name, slug)
    ...
    return new Promise(async (resolve, reject) => {
      findByID(id).then((findResult) => {
        if (findResult && findResult.length >= 1) {
          return reject(new Error(`Platform Family with id '${id}' already exists.`));
        }
      }).catch(() => {
        pool.query(createPlatformFamilyQuery).then((createResult) => {
