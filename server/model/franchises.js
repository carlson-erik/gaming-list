...
async function findByID(id) {
  ...
  return new Promise(async (resolve, reject) => {
    pool.query(findByIDQuery, [id]).then((findResult) => {
      ...
      reject(`Unable to find Franchise with ID: '${id}'`)
    }).catch(error => {
      reject(error)
    });
  });
...
async function create(id, name, slug, games) {
  const createFranchiseQuery = `
    INSERT INTO "franchises".index (id, name, slug, games)
    VALUES ('${id}', '${name}', '${slug}', '${getSQLValuesArray(games, 'INTEGER')}')
    RETURNING id;
  `;
  ...
  return new Promise(async (resolve, reject) => {
    findByID(id).then((findResult) => {
      if (findResult && findResult.length >= 1) {
        return reject(new Error(`Franchise with id '${id}' already exists.`));
      }
    }).catch(() => {
      pool.query(createFranchiseQuery).then((createResult) => {
